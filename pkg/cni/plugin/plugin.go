...
// GRPCClient is an interface for the client which is used to communicate with NVIDIA IPAM Daemon
//
//go:generate mockery --name GRPCClient

type GRPCClient interface {
	daemonv1.IPAMBackendServiceClient
}
...
func (p *Plugin) CmdAdd(args *skel.CmdArgs) error {
	...
	ctx, cFunc := context.WithTimeout(context.Background(),
		time.Second*time.Duration(cmd.Config.IPAM.DaemonCallTimeout))
	defer cFunc()
	resp, err := cmd.Client.Allocate(ctx, &daemonv1.AllocateRequest{Parameters: cmd.ReqParams})
	if err != nil {
...
func (p *Plugin) CmdDel(args *skel.CmdArgs) error {
	...
	ctx, cFunc := context.WithTimeout(context.Background(),
		time.Second*time.Duration(cmd.Config.IPAM.DaemonCallTimeout))
	defer cFunc()
	if _, err := cmd.Client.Deallocate(ctx, &daemonv1.DeallocateRequest{Parameters: cmd.ReqParams}); err != nil {
		return log.Errorf("grpc call failed: %v", err)
	}
...
func (p *Plugin) CmdCheck(args *skel.CmdArgs) error {
	...
	ctx, cFunc := context.WithTimeout(context.Background(),
		time.Second*time.Duration(cmd.Config.IPAM.DaemonCallTimeout))
	defer cFunc()
	if _, err := cmd.Client.IsAllocated(ctx, &daemonv1.IsAllocatedRequest{Parameters: cmd.ReqParams}); err != nil {
		return log.Errorf("grpc call failed: %v", err)
	}
...
type cmdContext struct {
	...
	ReqParams *daemonv1.IPAMParameters
}
...
func grpcRespToResult(resp *daemonv1.AllocateResponse) (*current.Result, error) {
...
func cniConfToGRPCReq(conf *types.NetConf, args *skel.CmdArgs) (*daemonv1.IPAMParameters, error) {
...
func defaultNewGRPCClientFunc(daemonSocket string) (GRPCClient, error) {
	...
	return daemonv1.NewIPAMBackendServiceClient(conn), nil
}
