...
const (
	...
	DefaultDaemonSocket = "unix:///var/lib/cni/nv-ipam/daemon-socket"
	// DefaultDaemonCallTimeout is the default timeout IPAM daemon calls
	DefaultDaemonCallTimeout = 5
...
type IPAMConf struct {
	...
	DaemonCallTimeout int    `json:"daemonCallTimeout,omitempty"`
...
func parsePoolName(poolName string) ([]string, error) {
	...
	if len(pools) == 0 {
		return nil, fmt.Errorf("pool list can't be empty")
	}
	if len(pools) > 2 {
		return nil, fmt.Errorf("pool list can't contain more then two entries")
	}
	for _, p := range pools {
		if p == "" {
			return nil, fmt.Errorf("invalid pool list format")
		}
	}
	return pools, nil
}
...
func (cl *confLoader) overlayConf(from, to *IPAMConf) {
	...
	if to.DaemonCallTimeout == 0 {
		to.DaemonCallTimeout = from.DaemonCallTimeout
	}
}
