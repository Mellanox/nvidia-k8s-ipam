...
func NewIPAllocator(s *RangeSet, poolName string, store storePkg.Store) IPAllocator {
	return &allocator{
		rangeSet: s,
		store:    store,
		poolName: poolName,
...
func (a *allocator) Allocate(id string, ifName string, meta types.ReservationMetadata) (*current.IPConfig, error) {
	...
	for {
		...
		err := a.store.Reserve(a.poolName, id, ifName, meta, reservedIP.IP)
		if err == nil {
			break
		}
...
func (a *allocator) getIter() *RangeIter {
	...
	lastReservedIP := a.store.GetLastReservedIP(a.poolName)
...
// StartIP returns start IP of the range
