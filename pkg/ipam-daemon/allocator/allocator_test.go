...
func mkAlloc(store storePkg.Store) allocator.IPAllocator {
	p := allocator.RangeSet{
		allocator.Range{Subnet: mustSubnet("192.168.1.0/29")},
	}
	Expect(p.Canonicalize()).NotTo(HaveOccurred())
	return allocator.NewIPAllocator(&p, testPoolName, store)
}

func newAllocatorWithMultiRanges(store storePkg.Store) allocator.IPAllocator {
	p := allocator.RangeSet{
...
func (t AllocatorTestCase) run(idx int, store storePkg.Store) (*current.IPConfig, error) {
	...
	for r := range t.ipMap {
		Expect(store.Reserve(testPoolName, r, "net1", types.ReservationMetadata{}, net.ParseIP(r))).NotTo(HaveOccurred())
	}
	store.SetLastReservedIP(testPoolName, net.ParseIP(t.lastIP))

	Expect(p.Canonicalize()).To(Succeed())
	alloc := allocator.NewIPAllocator(&p, testPoolName, store)
...
var _ = Describe("allocator", func() {
	Context("RangeIter", func() {
		It("should loop correctly from the beginning", func() {
			store, err := storePkg.NewManager(filepath.Join(GinkgoT().TempDir(), "test_store")).Open(context.Background())
