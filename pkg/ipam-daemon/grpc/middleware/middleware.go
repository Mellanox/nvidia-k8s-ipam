...
func LogRequestMiddleware(ctx context.Context, req interface{},
	...
	klog.FromContext(ctx).Info("REQUEST")
...
func LogResponseMiddleware(ctx context.Context, req interface{},
	_ *grpc.UnaryServerInfo, handler grpc.UnaryHandler) (interface{}, error) {
	reqLogger := klog.FromContext(ctx)
	resp, err := handler(ctx, req)
	startTime, ok := ctx.Value(startTimeKey{}).(time.Time)
	if ok {
		reqLogger = reqLogger.WithValues("call_duration_sec", time.Since(startTime).Seconds())
	}
	if err != nil {
